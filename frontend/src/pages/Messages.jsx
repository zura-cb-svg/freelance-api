...
export function Messages() {
  ...
  useEffect(() => {
    ...
    // 1. მოგვაქვს ისტორია და მოსაუბრის ნამდვილი სახელი API-დან
    const fetchHistory = async () => {
      try {
        const baseUrl = import.meta.env.DEV 
          ? "http://127.0.0.1:8000" 
          : "https://freelance-api-g8gh.onrender.com";
        
        const res = await fetch(`${baseUrl}/chat/history/${user.id}/${receiverId}`);
  ...
  const sendMessage = (e) => {
    ...
    ws.current.send(JSON.stringify({ content: inputValue, receiver_id: receiverId }));
    setMessages((prev) => [...prev, { sender_id: user.id, content: inputValue }]);
    setInputValue("");
  };

  if (!user) {
    return <div className="container-page py-10 text-center">Please log in to view messages.</div>;
  }
        ...
        {!receiverId ? (
          <div className="m-auto p-10">
            ...
            <div className="text-center mt-4">
              <Link to="/jobs" className="btn-primary px-4 py-2 text-sm inline-block">Browse Jobs</Link>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
              {messages.length === 0 && (
                <div className="text-center text-ink-400 text-sm mt-10">No messages yet. Say hello! 👋</div>
              )}
                ...
                return (
                  <div key={idx} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 text-[15px] ${
                      isMe ? "bg-brand-600 text-white rounded-br-sm" : "bg-white border border-line text-ink-900 rounded-bl-sm shadow-sm"
                    }`}>
                      {msg.content}
                    </div>
                  </div>
